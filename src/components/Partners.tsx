  ...
  return (
    <div className="py-20 max-w-screen-lg mx-auto" ref={container}>
      <div className="mb-10 p-4">
        <h1 className="main-title">Chosen by componies at</h1>
      </div>
      <div className="relative overflow-hidden whitespace-nowrap py-4 group">
        <div className="logo-slide bg-background inline-block animate-slide-logo group-hover:[animation-play-state:paused]">
          {partners.map((partner) => (
            <img
              ...
              alt={partner.name}
              key={partner.name}
              className="inline-block h-[30px] w-auto mx-8 filter grayscale object-contain"
            />
          ))}
        </div>
        <div className="logo-slide bg-background inline-block animate-slide-logo group-hover:[animation-play-state:paused]">
        ...
        <div className="absolute left-0 top-0 h-full w-[50px] bg-gradient-to-l from-transparent to-background" />
        <div className="absolute right-0 top-0 h-full w-[50px] bg-gradient-to-r from-transparent to-background" />
      </div>
    </div>
  );
};
