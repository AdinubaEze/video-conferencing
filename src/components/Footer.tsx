    ...
    <div className="py-20 px-4 max-w-screen-lg mx-auto" ref={container}>
      <div className="flex items-center justify-center flex-col gap-5 px-4 py-10 rounded-2xl shadow-card-inset newsletter">
        ...
        <div className="flex bg-secondary p-2 rounded-2xl shadow-card-inset form">
          <input
            ...
            className="bg-transparent w-full border-none outline-none h-[40px] px-3"
          />
      ...
      <div className="grid grid-cols-1 gap-6 mt-12 sm:grid-cols-2 md:grid-cols-4">
        <div>
          ...
          <p className="text-lg text-muted mt-2 text-center sm:text-left">
            ...
            aliquid laboriosam soluta perferendis nihil est
          </p>
        </div>
        {navigation.map((nav, index) => (
          <div className="space-y-2" key={index}>
            ...
            {nav.routes.map((route, index) => (
              <Link
                to={route.href}
                className="block text-lg text-muted text-center sm:text-left transition-all hover:text-white"
                key={index}
              >
                {route.label}
              </Link>
      ...
      <div className="flex items-center justify-between gap-4 flex-col md:flex-row py-6 px-4 mt-10 border-t border-white/10 shadow-card-inset rounded-3xl">
        <div className="flex items-center gap-4">
          {socialhandles.map((social, index) => (
            <Link
              to={social.href}
              target="_blank"
              className="rounded-icon shadow-card-inset hover:bg-primary/50 transition-all"
              key={index}
            >
              <social.icon size={15} />
            </Link>
        ...
        <div className="text-sm text-center">
          Copyright &copy; OneRoom 2024 - All rights reserved
        </div>
        <div className="flex items-center gap-4">
          <Link
            to={"#"}
            className="text-sm text-muted transition-all hover:text-primary hover:underline"
          >
            Term of use
          </Link>
          <Link
